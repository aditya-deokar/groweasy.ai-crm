...
export type PreviewValidationStatus =
  (typeof previewValidationStatusValues)[number]
export type AiGuardrailDecision = "ALLOW" | "WARN" | "BLOCK"
  ...
  return {
    ...
    file: {
      originalName: expectString(
        expectRecord(record.file, "preview.file").originalName,
        "preview.file.originalName"
      ),
      sizeBytes: expectNumber(
        expectRecord(record.file, "preview.file").sizeBytes,
        "preview.file.sizeBytes"
      ),
      sha256: expectString(
        expectRecord(record.file, "preview.file").sha256,
        "preview.file.sha256"
      ),
    },
    headers: expectStringArray(record.headers, "preview.headers"),
    previewRows: expectArray(record.previewRows, "preview.previewRows").map(
      (item) => parsePreviewRow(item)
    ),
...
export function parseImportStatus(value: unknown): ImportStatus {
  ...
  return {
    ...
    progress: {
      totalRows: expectNumber(
        expectRecord(record.progress, "status.progress").totalRows,
        "status.progress.totalRows"
      ),
      processedRows: expectNumber(
        expectRecord(record.progress, "status.progress").processedRows,
        "status.progress.processedRows"
      ),
      totalBatches: expectNumber(
        expectRecord(record.progress, "status.progress").totalBatches,
        "status.progress.totalBatches"
      ),
      completedBatches: expectNumber(
        expectRecord(record.progress, "status.progress").completedBatches,
        "status.progress.completedBatches"
      ),
      failedBatches: expectNumber(
        expectRecord(record.progress, "status.progress").failedBatches,
        "status.progress.failedBatches"
      ),
      percent: expectNumber(
        expectRecord(record.progress, "status.progress").percent,
        "status.progress.percent"
      ),
    },
    totals: {
      imported: expectNumber(
        expectRecord(record.totals, "status.totals").imported,
        "status.totals.imported"
      ),
      skipped: expectNumber(
        expectRecord(record.totals, "status.totals").skipped,
        "status.totals.skipped"
      ),
    },
    error: expectNullableString(record.error, "status.error"),
    aiSafety: parseAiSafetySummary(record.aiSafety),
    recentEvents: expectArray(record.recentEvents, "status.recentEvents").map(
      (item) => parseImportEvent(item)
    ),
  }
}
...
export function parseImportResult(value: unknown): ImportResult {
  ...
  return {
    ...
    summary: {
      totalRows: expectNumber(
        expectRecord(record.summary, "result.summary").totalRows,
        "result.summary.totalRows"
      ),
      totalImported: expectNumber(
        expectRecord(record.summary, "result.summary").totalImported,
        "result.summary.totalImported"
      ),
      totalSkipped: expectNumber(
        expectRecord(record.summary, "result.summary").totalSkipped,
        "result.summary.totalSkipped"
      ),
      totalBatches: expectNumber(
        expectRecord(record.summary, "result.summary").totalBatches,
        "result.summary.totalBatches"
      ),
      completedBatches: expectNumber(
        expectRecord(record.summary, "result.summary").completedBatches,
        "result.summary.completedBatches"
      ),
      failedBatches: expectNumber(
        expectRecord(record.summary, "result.summary").failedBatches,
        "result.summary.failedBatches"
      ),
      pendingBatches: expectNumber(
        expectRecord(record.summary, "result.summary").pendingBatches,
        "result.summary.pendingBatches"
      ),
      processingBatches: expectNumber(
        expectRecord(record.summary, "result.summary").processingBatches,
        "result.summary.processingBatches"
      ),
      processedRows: expectNumber(
        expectRecord(record.summary, "result.summary").processedRows,
        "result.summary.processedRows"
      ),
      unprocessedRows: expectNumber(
        expectRecord(record.summary, "result.summary").unprocessedRows,
        "result.summary.unprocessedRows"
      ),
    },
    batches: expectArray(record.batches, "result.batches").map((item) =>
      parseImportBatch(item)
    ),
    aiSafety: parseAiSafetySummary(record.aiSafety),
    skippedReasonCounts: parseNumberRecord(
      record.skippedReasonCounts,
      "result.skippedReasonCounts"
    ),
    events: expectArray(record.events, "result.events").map((item) =>
      parseImportEvent(item)
    ),
    records: expectArray(record.records, "result.records").map((item) =>
      parseImportedRecord(item)
    ),
    skippedRecords: expectArray(
      record.skippedRecords,
      "result.skippedRecords"
    ).map((item) => parseSkippedRecord(item)),
    pageInfo: {
      nextCursor: expectNullableNumber(
        expectRecord(record.pageInfo, "result.pageInfo").nextCursor,
        "result.pageInfo.nextCursor"
      ),
      hasMore: expectBoolean(
        expectRecord(record.pageInfo, "result.pageInfo").hasMore,
...
function parsePreviewRow(value: unknown): PreviewRow {
  ...
  const validationStatus = expectString(
    record.validationStatus,
    "previewRow.validationStatus"
  )

  if (
    !previewValidationStatusValues.includes(
      validationStatus as PreviewValidationStatus
    )
  ) {
...
function parsePreviewSummary(value: unknown): ImportPreview["summary"] {
  ...
  return {
    ...
    previewRowCount: expectNumber(
      record.previewRowCount,
      "preview.summary.previewRowCount"
    ),
    candidateRowCount: expectNumber(
      record.candidateRowCount,
      "preview.summary.candidateRowCount"
    ),
    skippedRowCount: expectNumber(
      record.skippedRowCount,
      "preview.summary.skippedRowCount"
    ),
    emptyRowCount: expectNumber(
      record.emptyRowCount,
      "preview.summary.emptyRowCount"
    ),
    duplicateRowCount: expectNumber(
      record.duplicateRowCount,
      "preview.summary.duplicateRowCount"
    ),
    noContactRowCount: expectNumber(
      record.noContactRowCount,
      "preview.summary.noContactRowCount"
    ),
    warningCount: expectNumber(
      record.warningCount,
  ...
  return {
    ...
    metadata:
      record.metadata == null
        ? null
        : expectRecord(record.metadata, "event.metadata"),
...
function parseImportBatch(value: unknown): ImportBatch {
  ...
  return {
    ...
    safetyEventCount: expectNumber(
      record.safetyEventCount,
      "batch.safetyEventCount"
    ),
    blockedRows: expectNumber(record.blockedRows, "batch.blockedRows"),
    warnedRows: expectNumber(record.warnedRows, "batch.warnedRows"),
  }
}

function parseAiSafetySummary(value: unknown): AiSafetySummary {
  if (value == null) {
    return emptyAiSafetySummary()
  }

  const record = expectRecord(value, "aiSafety")
  const provider = expectNullableString(record.provider, "aiSafety.provider")

  if (provider !== null && provider !== "gemini" && provider !== "openai") {
    throw new Error("aiSafety.provider is invalid.")
  }

  return {
    promptVersion: expectNullableString(
      record.promptVersion,
      "aiSafety.promptVersion"
    ),
    provider,
    model: expectNullableString(record.model, "aiSafety.model"),
    blockedRows: expectNumber(record.blockedRows, "aiSafety.blockedRows"),
    warnedRows: expectNumber(record.warnedRows, "aiSafety.warnedRows"),
    outputRejectedBatches: expectNumber(
      record.outputRejectedBatches,
      "aiSafety.outputRejectedBatches"
    ),
    safetyEvents: expectNumber(record.safetyEvents, "aiSafety.safetyEvents"),
  }
}

function emptyAiSafetySummary(): AiSafetySummary {
  return {
    promptVersion: null,
    provider: null,
    model: null,
...
function parseGrowEasyCrmRecord(value: unknown): GrowEasyCrmRecord {
  ...
  return {
    ...
    country_code: expectNullableString(
      record.country_code,
      "record.country_code"
    ),
    ...
    confidence:
      record.confidence == null
        ? null
        : parseNumberRecord(record.confidence, "record.confidence"),
  }
}
